    ...
    $.fn.bootstrapDualListbox = function (options) {
        ...
        return this.each(function () {
            ...
            var settings = $.extend({
                ...
                nonselectedlistlabel: true,            // 'string', false
                selectedlistlabel: true,             // 'string', false
                hdnControlId: ''
            }, options);
            ...
            function appendSelectedValue() {
                if (settings.hdnControlId != '') {
                    var selectedIds = '';
                    var filterText = elements.filterinput2.val();
                    elements.filterinput2.val('');
                    refreshselects();
                    elements.select2.find('option').each(function (index, item) {
                        selectedIds += ($(item).val() + ',');
                    });
                    $('input:hidden[id*="' + settings.hdnControlId + '"]').val(selectedIds);
                    elements.filterinput2.val(filterText);
                    refreshselects();
                }
            }
