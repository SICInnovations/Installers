 ...
 */
  $.fn.simpleColor = function(options) {
    ...
    function buildChooser(index) {
      ...
      var displayBox = $("<div class='simpleColorDisplay' contenteditable />");
      ...
      var selectCallback = function (event) {
        ...
        $('html').bind("click.simpleColorDisplay", function(e) {
          ...
          if (target.is('.simpleColorCell') === false || $.contains( $(event.target).closest('.simpleColorContainer')[0], target[0]) === false) {
            ...
            if (options.displayColorCode) {
                ...
                var returnVal = $(event.target).closest('.simpleColorDisplay')[0].innerHTML.match(/^#(([0-9a-fA-F]{2}){3}|([0-9a-fA-F]){3})$/) !== null;               
                if (returnVal==false)
                    color = '';             
                else 
                    color = $(event.target).closest('.simpleColorDisplay')[0].innerHTML;
        ...
        // Build the chooser.
          ...
          for (var i=0; i<options.colors.length; i++) {
            ...
            function(event) {
                var color = '#' + this.id
                var returnVal = color.match(/^#(([0-9a-fA-F]{2}){3}|([0-9a-fA-F]){3})$/) !== null;
                if (!returnVal && x.value != '')
                    color = '';
                event.data.input.value = color;
               
                var InputControl = container.parent().parent().find('input[type=hidden]').prop('id');
                var CheckFileUploadInputControl = container.parent().parent().find('input[id*=hdnImageUrl]').prop('id');
             
                // if (imgImageUrl)
                if (InputControl != null) {
                    if (CheckFileUploadInputControl == null || CheckFileUploadInputControl == undefined)                
                    $('#' + InputControl).val(color);                
                }
  ...
  $.fn.setColor = function(color) {
    this.each( function(index) {
        var displayBox = $(this).data('dvTier1').find('input');
